...
const VendorList: React.FC = () => {
  ...
  const handleEdit = (id: number) => {
    dispatch(getVendorDetails(id)).then((resp) => {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      if (modalMode === "add") {
        const result = await dispatch(createVendor({ name: formData.name }));
        ...
        const result = await dispatch(
          updateVendor({ id: vendorId, vendorData: formData }),
        );
          ...
          <Dialog.Panel className="mx-auto w-full max-w-md rounded-lg bg-white p-6">
            ...
            >
              ...
              <div>
                ...
                <select
                  name="status"
                  defaultValue={selectedVendor?.status?.toString() || "true"}
                  className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
